...
// Security utility for sanitizing log output
...
export default function Home() {
  ...
  const setRecordingWithDebug = useCallback((value: boolean) => {
    console.log(`[Recording State] Setting to ${sanitizeForLogging(value)}`);
    setRecording(value);
  }, []);
    ...
    wsRef.current.onmessage = (event) => {
      if (typeof event.data === 'string') {
        try {
          ...
          console.log('[WS MESSAGE]', sanitizeForLogging(msg));
          
          if (msg.event) handleEventMessage(msg.event);
        } catch (e) {
          console.error('Error parsing message:', sanitizeForLogging(e));
        }
      }
    };
  };
  ...
  const handleAudioError = useCallback((error: Error) => {
    console.error('[Audio Error]', sanitizeForLogging(error));
    setRecordingWithDebug(false);
    ...
    if (event.contentStart && event.contentStart.type === 'TEXT') {
      ...
      if (event.contentStart.additionalModelFields) {
        try {
          ...
          console.error('[TEXT STAGE ERROR]', sanitizeForLogging(e));
        }
      }
      if (contentId) {
        console.log('[TEXT STAGE]', sanitizeForLogging(contentId), sanitizeForLogging(stage));
        contentIdToStage.current[contentId] = stage;
      }
    } else if (event.contentStart && event.contentStart.type === 'TOOL') {
      console.log('[TOOL START]', sanitizeForLogging(event));
      setWaitingForTool(true);
    } else if (event.toolUse) {
      console.log('[TOOL USE]', sanitizeForLogging(event.toolUse));
      setWaitingForTool(true);
      ...
      console.log('[TOOL RESULT FULL]', sanitizeForLogging(event.toolResult));
      try {
        const parsedContent = JSON.parse(event.toolResult.content);
        console.log('[TOOL RESULT PARSED]', sanitizeForLogging(parsedContent));
      } catch (e) {
        console.error('[TOOL RESULT PARSE ERROR]', sanitizeForLogging(e));
      }
        ...
        <Card className="flex-1 shadow-xl border-0 p-8 space-y-6 bg-white h-full flex flex-col">
          <div className="flex items-center justify-between mb-2 border-b pb-4 border-gray-100">
            ...
            <span className="ml-2 flex items-center gap-2">
              ...
              <span className="inline-block w-3 h-3 rounded-full bg-green-500" title={status}></span>
            </span>
          </div>
